...
import useNotificationFilter from "../hooks/useFilterNotification";

const Menubar = () => {
  const notification=useNotification((state)=>state.notification)
  const removeAllNotifications=useNotification(state=>state.removeAllNotifications)
  const navigate=useNavigate('/')
  const route=useRoute(state=>state.route)
  const changeRoute=useRoute(state=>state.changeRoute)
  const user=useAuth(state=>state.user)
  const { bookmark:isAddingBookmarks, message:isAddingMessage, friend:isAddingFriends  }=useNotificationFilter(notification)


  const handleToRoute=(value)=>{
    if(value.includes("bookmark")){
      removeAllNotifications("bookmark")
    }
    changeRoute(value)
  }
  useEffect(()=>{
  ...
  return (
    <div className="flex flex-row items-center md:flex-col fixed bottom-0 md:top-0 left-0 md:w-14 md:px-0 px-3 w-full md:h-full h-16 bg-slate-900 z-30">
      <div className="flex-1 flex flex-row md:flex-col items-center justify-center gap-5">
        ...
        <ButtonIcon onClick={()=>handleToRoute('/message')} text={isAddingMessage>0?isAddingMessage.toString():""} textClassName={'lg:text-[15px] absolute -top-2 right-2 px-1.5 bg-red-500 rounded-full'}  className={`relative w-full flex items-center justify-center py-2 md:px-2 text-2xl text-slate-100 rounded-md  ${route==="/message"?"shadow-lg bg-slate-100 text-slate-900":"hover:shadow-lg hover:bg-slate-100 hover:text-red-600"} transition-all duration-500`}>
          <AiFillMessage />
        </ButtonIcon>
        <ButtonIcon onClick={()=>handleToRoute('/friends')} text={isAddingFriends>0?isAddingFriends.toString():""} textClassName={'lg:text-[15px] absolute -top-2 right-2 px-1.5 bg-red-500 rounded-full'}  className={`relative w-full flex items-center justify-center py-2 md:px-2 text-2xl text-slate-100 rounded-md ${route==="/friends"?"shadow-lg bg-slate-100 text-slate-900":"hover:shadow-lg hover:bg-slate-100 hover:text-red-600"} transition-all duration-500`}>
          <FaUserFriends />
        </ButtonIcon>
        <ButtonIcon onClick={()=>handleToRoute('/bookmarks')} text={isAddingBookmarks>0?isAddingBookmarks.toString():""} textClassName={'lg:text-[15px] absolute -top-2 right-2 px-1.5 bg-red-500 rounded-full'}  className={`relative w-full flex items-center justify-center py-2 md:px-2 text-2xl text-slate-100 rounded-md  ${route==="/bookmarks"?"shadow-lg bg-slate-100 text-slate-900":"hover:shadow-lg hover:bg-slate-100 hover:text-red-600"} transition-all duration-500`}>
          <IoBookmarks />
        </ButtonIcon>
